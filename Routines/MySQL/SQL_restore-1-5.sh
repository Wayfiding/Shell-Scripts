...
ssh ${1}@${2}  'mysql -u '${3}' -p'${4}'  -e "use  '${5}'" -v ' &> /dev/null
if [[ "${?}" -eq 0 ]]
then 
ssh ${1}@${2} 'mysql -u '${3}' -p'${4}'  -e "drop database '${5}'" -v;
mysql -u  '${3}' -p'${4}'  -e "create database '${5}'" -v'
  ...
  if [[ "${CREATE_DATA}" = 'yes' ]]
  then
  ssh ${1}@${2} 'mysql -u '${3}' -p'${4}'  -e "create database '${5}'" -v'
...
ssh ${1}@${2}  'mysql -u '${3}' -p'${4}' '${5}' -v <  '${BACKUP_PATH}'' > /home/dump.log
...
#/home/vagrant/backup/backup.sql